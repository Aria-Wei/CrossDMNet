...
def pairwise_distance_torch(embeddings, device):
    ...
    return pairwise_distances # 返回一个距离平方矩阵（去除了对角线）
...
def TripletSemiHardLoss(y_true, y_pred, device, margin=1.0):
    ...
    triplet_loss = (torch.max(torch.mul(loss_mat, mask_positives), torch.tensor([0.]).to(device))).sum() / (num_positives + 1e-8)
